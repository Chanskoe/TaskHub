...
class _CalendarViewState extends State<CalendarView> {
  ...
  int _taskStatusPriority(TaskModel task) {
    if (task.isOverdue() && !task.isCompleted) return 0; // просроченные
    if (!task.isCompleted) return 1; // невыполненные
    return 2; // выполненные
  }
  ...
  List<TaskModel> _getTasksForDay(DateTime day) {
    ...
    // Сортировка: просроченные → невыполненные → выполненные, затем по дате дедлайна
